...
2. The service selects only synchronized and fresh IDR books whose applicable side/denomination input rule, outcome rule, and market price rule are verified. A legacy venue may use one base-quantity rule only where that accurately represents its contract.
3. For a buy whose venue input is base quantity, it walks asks up to the IDR budget, floors the derived base quantity to the verified rule, and re-walks. For a quote-denominated buy such as Reku, it quantizes the IDR input, walks that budget directly, and applies any separately verified conservative outcome rule without fabricating unspent IDR through a base-quantity re-walk.
4. For a sell, it floors the requested base quantity to the venue's verified sell-input step before walking bids. The response reports the unsold base remainder separately from an insufficient-depth remainder.
5. Existing order-book prices are validated against their verified venue steps and never silently rounded. Aggregate quantities are checked against an execution step only when the venue publishes step-aligned quantities; quantities reported or derived from quote notional use explicit book semantics and retain structural/decimal validation without an inapplicable modulo check. The weighted average is derived analytics and need not lie on the price tick.
