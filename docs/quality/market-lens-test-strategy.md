...
- Market discovery at `/markets` and pair detail at `/markets/{pair}`, including direct navigation, browser history, same-pair last-price comparison, price/depth/trade panels, and one overlaid multi-venue OHLC chart.
...
| Markets web routes | Local production build, fixture BFF | Playwright Chromium desktop, Pixel 5, 320 px reflow, 200% zoom, reduced motion | Deterministic multi-venue market, OHLC, order-book, and trade scenarios with route interception |
| Markets read APIs | In-process integration and built BFF | Supertest and Playwright `APIRequestContext` | Frozen clock; exact decimal strings; healthy, partial, stale, gapped, malformed, unsupported, and recovery fixtures |
...
| AC-25 normalized overlay | Common-baseline percent math; series eligibility | Identical pair/range/interval per venue | Absolute OHLC plus normalized value | One plot, labelled series, legend, shared inspection | Keyboard, contrast, accessible data equivalent | All series rebase at same valid common bucket |
...
| AC-27 fixed chart/state | Shared 24h/1h bounds and series visibility state | Aggregate snapshot always returns fixed bounds | Response interval fixed to `1h` | No range control; refresh/error/focus/visibility | Announcement/reduced-motion review | Fixed range and visibility survive refresh |
...
| Markets route correctness | AC-17–AC-29 deterministic/API/browser suites | 100% pass; no open P0/P1; every unsupported or unhealthy capability fails closed |
| Markets accessibility | Automated accessibility scan plus keyboard/reflow/chart-data-alternative evidence | Zero critical/serious violations; all information available without pointer, color, animation, or visual chart inspection |
...
| AC-25 | At least two aligned valid OHLC series with different absolute prices | The default 24-hour chart renders, a venue toggles, or a timestamp is inspected | One plot contains labelled close-movement series sharing time and normalized-percentage axes, all rebased to zero at the same first common valid bucket. The legend persists and shared pointer/keyboard inspection exposes each venue's absolute O/H/L/C and normalized change. |
...
| AC-27 | A fixed 24-hour/1-hour aggregate detail fixture plus a delayed background refresh | The candle component loads and refreshes | Only the server-defined 24-hour range with 1-hour buckets is displayed; no range control is rendered. Series visibility, prior valid chart, focus, and accessible summary persist while loading/error is announced. |
...
AC-25 deliberately defines the default as normalized percentage movement: close is the compared line, every included venue shares one valid common baseline bucket, and full absolute OHLC remains inspectable. Absolute-IDR and single-venue candlestick modes are deferred. Silently rebasing venues at different timestamps, mixing raw prices with normalized values, or including a venue without the common baseline is a P1 defect.
...
- Integration tests cover phase-1 `GET /api/markets` and aggregate `GET /api/markets/:pair`. They verify supported/unsupported pair validation, canonical pair echo, fixed `1h` interval and 24-candle bound, per-component partial success, bounded upstream timeouts, malformed/oversized payloads, and recovery. An invalid pair must be rejected before adapter fan-out; an overview refresh uses at most one batched ticker operation per venue, not one call per row.
- Black-box `APIRequestContext` coverage verifies `400 INVALID_REQUEST`, `404 MARKET_NOT_FOUND`, product `429`, and projection-level `503` behavior; a successfully built aggregate remains `200` when individual venue components are unhealthy. It also checks content type, cache policy, schema version, exact numeric strings, stable sorting, canonical venue order, pair validation, fixed interval/bounds, cache variation, and non-sensitive errors.
...
2. Load a supported detail URL directly and prove pricing, order book, public trades, and the single three-venue OHLC overlay all identify the URL pair.
...
7. Verify there is no phase-1 range control; operate venue-series visibility controls by keyboard, inspect the accessible OHLC alternative and multi-series timestamp values, and verify a missing venue candle remains a gap. A targeted screenshot may supplement this check but cannot replace semantic assertions.
